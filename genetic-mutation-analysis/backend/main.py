...
evo2_image = (
    ...
    .apt_install(
        ["build-essential", "cmake", "ninja-build", "libcudnn8-dev", "git", "gcc", "g++"]
    )
    .env({
        "CC": "/usr/bin/gcc",
        "CXX": "/usr/bin/g++",
    ...
    .run_commands(
        "sed -i 's/np.fromstring/np.frombuffer/g' /evo2/vortex/vortex/model/tokenizer.py && "
        "sed -i 's/(text, dtype=np.uint8)/(text.encode(), dtype=np.uint8)/g' /evo2/vortex/vortex/model/tokenizer.py"
    )
)
...
def brca1_example():
    ...
    if "plot" in result:
        ...
        with open("brca1_evo2_plot.png", "wb") as f:
            f.write(plot_data)
